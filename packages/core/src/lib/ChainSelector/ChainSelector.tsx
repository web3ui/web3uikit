...
const ChainSelector: FC<IChainSelectorProps> = ({
  ...
  const getChainNameById = (chainId: string) => providers.find((provider) => provider.chainId === chainId)?.chain;
  ...
  const isAllSelected = useMemo(() => {
    ...
    if (!isCompatibilityChecked) {
      ...
      providers.forEach((provider) => {
        if (isCompatibilityChecked && availableChains.includes(provider.chain)) {
          return;
  ...
  const checkIncompatibleChains = (providerOption: OptionType) => {
    ...
    return !!providers
      .filter((o) => values.map((x) => x.chainId).includes(o.chainId) && o.chainId !== providerOption.chainId)
  ...
  const handleBatchSelect = () => {
    if (isAllSelected) setValue([]);
      ...
      providers.forEach((provider) => {
        if (isCompatibilityChecked && addedChains.includes(provider.chain)) {
          return;
        }
      ...
      {providers?.length > 0 ? (
        <GridStyled>
          {providers.map((option) => (
            <GridElementStyled key={`crypto_card-${option.chainId}`}>
              <Card
                title={option.chain}
                description={option.name}
                onClick={() => toggleEvm(option.chainId)}
                isSelected={values.map((x) => x.chainId).includes(option.chainId)}
                isDisabled={checkIncompatibleChains(option)}
              >
                <CardContentStyled>
                  <CryptoLogos chain={getChainLogoName(option.chainId as chainIdType)} />
                </CardContentStyled>
              </Card>
            </GridElementStyled>
          ))}
