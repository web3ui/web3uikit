...
const normalizedAppRoot = app_root_1.workspaceRoot.replace(/\\/g, '/');
let tsModule;
...
function getCompilerHost(tsConfigPath) {
    ...
    const moduleResolutionCache = tsModule.createModuleResolutionCache(app_root_1.workspaceRoot, host.getCanonicalFileName);
    return { options, host, moduleResolutionCache };
}
function getRootTsConfigFileName() {
    for (const tsConfigName of ['tsconfig.base.json', 'tsconfig.json']) {
        const tsConfigPath = (0, path_1.join)(app_root_1.workspaceRoot, tsConfigName);
...
function getRootTsConfigPath() {
    ...
    return tsConfigFileName ? (0, path_1.join)(app_root_1.workspaceRoot, tsConfigFileName) : null;
}
