...
class FileHasherBase {
    ...
    hashFile(path) {
        ...
        const relativePath = (0, path_1.normalizePath)(path.startsWith(app_root_1.workspaceRoot)
            ? path.slice(app_root_1.workspaceRoot.length + 1)
            : path);
        if (this.fileHashes.has(relativePath)) {
            ...
            try {
                ...
                return hashing_impl_1.defaultHashing.hashFile((0, path_1.joinPathFragments)(app_root_1.workspaceRoot, relativePath));
            }
            catch (_a) {
                return '';
            }
