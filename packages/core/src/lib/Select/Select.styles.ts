...
const LabelStyled = styled.label<LabelProps>`
  ...
  height: 24px;
  left: ${({ hasPrefixIcon, hasSelectedIndex }) => (hasPrefixIcon && !hasSelectedIndex ? 50 : 12)}px;
  line-height: 1;
  pointer-events: none;
  position: absolute;
  padding: 0 4px;
  transition: all 0.1s ease-out;
  ${({ hasSelectedIndex }) => (hasSelectedIndex ? 'top: -9px; font-size: 14px;' : 'top: 20px;')};
`;
...
const SelectedItem = styled.div<SelectedItemProps>`
  ...
  ${({ state }) =>
    state === 'disabled' && `& * {fill: ${color.greyDisabled};} color: ${color.greyDisabled} !important;`};
