...
const Table: React.FC<TableProps> = ({
  ...
  const [pageNum, setPageNum] = useState<number>(customPageNumber ? customPageNumber : 0);
  ...
  const handleSortingChange = (fieldId: number) => {
    if (isColumnSortable.length !== 0 && isColumnSortable.length > fieldId && !isLoading && isColumnSortable[fieldId]) {
      const sortOrder = fieldId === sortField && order === 'asc' ? 'desc' : 'asc';
      ...
      <>
        {header.map((col, key) => (
          <>
            ...
            >
              {col}
              {sortField === key && <Icon svg={order === 'asc' ? 'triangleUp' : 'triangleDown'} fill={color.grey} />}
            </DivTableCell>
          </>
        ))}
        <Divider />
  ...
  const RenderTable = (): JSX.Element => {
    ...
    return (
      <>
        {tableData &&
          computeCurrentData().map((row: (string | React.ReactNode)[], rowKey, arr) => (
            <React.Fragment key={`fragment_${rowKey}`}>
              {row.map((item: string | React.ReactNode, colKey: number, rowData) => (
                <DivTableCell
                  key={`tr_${rowKey}_${colKey}`}
                  role='table-item'
                  className={`${colKey == 0 && 'firstCol'} ${colKey == rowData.length - 1 && 'lastCol'}`}
                  alignCellItems={alignCellItems}
                  justifyCellItems={justifyCellItems}
                >
                  {item}
              ...
              {rowKey != arr.length - 1 && <Divider key={`divider_${rowKey}`} />}
            </React.Fragment>
          ))}
      </>
    );
  ...
  const RenderPagination = (): JSX.Element => {
    ...
    return (
      <Pagination>
        <div>
          <PaginationText isActive={pageNum != 0} onClick={handlePrev} data-testid='pagination-prev'>
            Prev
          </PaginationText>
          {paginate(tableData?.length, pageNum, pageSize, maxPages).map((key) => (
            <PaginationTag
              key={`pagination_${key}`}
          ...
          <PaginationText
            isActive={pageNum + 1 < Math.ceil(tableData?.length / pageSize)}
            onClick={handleNext}
  ...
  const Loader = () => (
    <DivSpinnerLoaderParent>
      {customLoadingContent ? (
        ...
        <>
          <Loading spinnerType={'wave'} spinnerColor={'#B0B5BF'} size={6} />
