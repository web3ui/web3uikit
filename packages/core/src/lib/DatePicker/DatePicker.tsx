...
const { DivWrapperStyled, InputStyled, LabelStyled, StrongStyled } = InputStyles;
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  const hasValidation = () =>
    Boolean(validation?.required || validation?.max || validation?.min || validation?.required);
  ...
  return (
    ...
    >
      <InputStyled
        ...
        min={validation?.min}
        name={name}
        onBlur={(event: React.FocusEvent<HTMLInputElement>) => validate(event)}
      ...
      {label && (
        <LabelStyled data-testid='date-picker_label' hasPrefix={false} htmlFor={id} labelBgColor='white'>
          {label}
        </LabelStyled>
      )}

      <SpanStyled data-testid='date-picker_icon'>
        <Icon svg={iconTypes.calendar} fill={color.greyDark} size={24} />
      </SpanStyled>

      {currentState === 'error' && (
        <StrongStyled data-testid='date-picker_feedback' isError={false}>
          {invalidMessage}
        </StrongStyled>
      )}
    </DivWrapperStyled>
