...
const VerifyCode: FC<VerifyCodeProps> = ({
  ...
  const processInput = (e: ChangeEvent<HTMLInputElement>, idx: number) => {
    const newCode = [...code];
    const num = newCode[idx] === e.target.value[0] ? e.target.value[1] : e.target.value[0];
    newCode[idx] = num || '';
    setCode(newCode);
    if (idx !== length - 1) {
  ...
  return (
    <div className='VerifyCode' data-testid='test-VerifyCode'>
      <Typography variant='subtitle2' weight='semibold' color={color.black}>
        {label}
      </Typography>
      <DivStyledWrapper className='verify-code' data-testid='test-verify-code-id'>
        {code.map((num, idx) => (
          <InputStyled
      ...
      {state === 'error' && errorMessage !== '' && <DivStyledTooltip>{errorMessage}</DivStyledTooltip>}
    </div>
  );
};
