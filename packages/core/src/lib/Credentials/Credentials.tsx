...
const { CredentialsStyled, DividerStyled, DivWrapperStyled, PreformattedStyled, ToolsStyled } = styles;
...
const Credentials: FC<ICredentialsProps> = ({
  ...
  const [isMultiline, setIsMultiline] = useState((text.match(/\n/g) || []).length > 0);
  ...
  return (
    <CredentialsStyled width={width} data-testid='test-credentials'>
      ...
      />
      <PreformattedStyled>
        <DivWrapperStyled isHidden={isValueHidden} isMultiline={isMultiline}>
          <Typography
            monospace
          ...
          >
            {isValueHidden ? (
              ...
              <TruncateString text={text} percentageOfCharsAfterTrunc={55} />
            )}
          </Typography>
        </DivWrapperStyled>
        <ToolsStyled>
          {hasHideButton && <HideButton onToggle={() => setIsValueHidden(!isValueHidden)} isHidden={isValueHidden} />}
