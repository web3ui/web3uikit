...
function loadNxPlugins(plugins, paths = [workspace_root_1.workspaceRoot]) {
                ...
                try {
                    ...
                    if (e.code === 'MODULE_NOT_FOUND') {
                        ...
                        if (plugin) {
                            ...
                            pluginPath = main
                                ? path.join(workspace_root_1.workspaceRoot, main)
                                : plugin.path;
                        }
                        else {
...
function readPluginPackageJson(pluginName, paths = [workspace_root_1.workspaceRoot]) {
    try {
        const result = (0, package_json_1.readModulePackageJson)(pluginName, paths);
        return {
