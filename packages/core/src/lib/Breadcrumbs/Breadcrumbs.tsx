...
const { Breadcrumb, BreadcrumbsSeparator, ListItemStyled, ListStyled, NavStyled } = styles;
...
function renderList(routes: Route[], separator?: React.ReactNode, currentLocation?: string) {
  let separatedRoutes: any[] = [];
  routes.forEach((route, i) => {
    const crumb = (
      <ListItemStyled key={`breadcrumb-${i}`} data-testid={i == 0 ? 'breadcrumb-test-id' : `breadcrumb-test-id-${i}`}>
    ...
    const routesWithSeparator = getNumberOfRoutesToRender(routes, currentLocation);
    if (i < routesWithSeparator) {
      separatedRoutes = separatedRoutes.concat(
        crumb,
        <BreadcrumbsSeparator key={`separator-${i}`} data-testid={'breadcrumbs-separator-test-id'}>
          {separator ?? <Icon svg={iconTypes.chevronRight} fill='currentColor' size={24} />}
...
const Breadcrumbs: IBreadcrumbs = ({
  ...
  return (
    <NavStyled color={theme} data-testid={'breadcrumbs-nav-test-id'} {...props}>
