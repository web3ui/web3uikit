...
test('Radios - RadioGroup', async () => {
  ...
  render(<RadioGroup onChange={(event: React.ChangeEvent<HTMLInputElement>) => testEvent(event.target)} />);

  // renders the component
  const element = screen.getByTestId(testFieldsetId) as unknown as HTMLFieldSetElement | null;
  ...
  // renders the title
  const title = screen.getByTestId(testLegendId) as unknown as HTMLLegendElement | null;
  expect(title?.textContent).toBe(testTitle);

  // renders the correct amount of radio inputs
  const inputs = screen.getAllByRole('radio') as unknown as HTMLInputElement[];
  expect(inputs.length).toBe(testItems?.length);

  // renders the each input correctly
  testItems && inputs.forEach((input: HTMLInputElement, i) => expect(input?.value).toBe(String(i)));

  // renders the correct label for each input
  const labels = screen.getAllByRole('label') as unknown as HTMLLabelElement[];
  testItems && labels.forEach((label: HTMLLabelElement, i) => expect(label?.textContent).toBe(testItems[i]));

  // When the label is clicked the input is checked to render new styles
  const label: HTMLLabelElement | undefined = labels[0];
  expect(label).not.toBeNull();
  label && fireEvent.click(label);

  const input: HTMLInputElement | undefined = inputs[0];
...
test('Radios - RadioGroupWithoutTitle', async () => {
  ...
  const testItems = RadioGroupWithoutTitle?.args?.items && [...RadioGroupWithoutTitle.args.items];

  render(<RadioGroupWithoutTitle onChange={(event: React.ChangeEvent<HTMLInputElement>) => testEvent(event.target)} />);
  ...
  // renders no title
  const title = (await screen.queryByText(testLegendId)) as unknown as HTMLLegendElement | null;
...
test('Radios - RadiosWithLongText', async () => {
  ...
  const testItems = RadiosWithLongText?.args?.items && [...RadiosWithLongText.args.items];

  render(<RadiosWithLongText onChange={(event: React.ChangeEvent<HTMLInputElement>) => testEvent(event.target)} />);
...
test('Radios - RadiosSetParticular', async () => {
  ...
  const testItems = RadiosSetParticular?.args?.items && [...RadiosSetParticular.args.items];

  render(<RadiosSetParticular onChange={(event: React.ChangeEvent<HTMLInputElement>) => testEvent(event.target)} />);
  ...
  const defaultChecked = testIsChecked && (inputs[testIsChecked] as unknown as HTMLInputElement);
...
it('Adds Suffix to Radio', () => {
  const { getByTestId } = render(
    <RadiosSetParticular
      onChange={(event: React.ChangeEvent<HTMLInputElement>) => testEvent(event.target)}
