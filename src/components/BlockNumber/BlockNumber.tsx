...
const BlockNumber: React.FC = () => {
    ...
    const [{ chainID, block }, setChainBlock] = useState<{ chainID?: string; block?: number }>({ chainID: String(chainId) })
    ...
    useEffect(() => {
      ...
      if(chainId && web3){
        ...
        setChainBlock((chainBlock) => (chainBlock.chainID === chainId ? chainBlock : { chainID: chainId }))

        web3?.getBlockNumber().then(onBlock).catch((error) => {
    ...
    const value = useMemo(
      () => ({
        ...
        fastForward: (block: number) => setChainBlock({ chainID: String(chainId), block }),
      }),
