...
describe('Table - DefaultTable', () => {
  ...
  const pageSize = DefaultTable?.args?.pageSize ? DefaultTable?.args?.pageSize : 0;
  const maxPages = DefaultTable?.args?.maxPages ? DefaultTable?.args?.maxPages : 0;
  ...
  it('Should call back on page change', () => {
    const { rerender } = render(<DefaultTable onPageNumberChanged={testOnChangeEvent} />);
  ...
  it('Should change pagination to next on Click', () => {
    ...
    const firstPaginationKey: string = `${element[keys[0] as string].return.key}`;
    fireEvent.click(next);
    rerender(<DefaultTable />);
    element = screen.getByTestId('pagination_true');
    const Skeys: any = [...Object.keys(element)];
    const SeccondPaginationKey: string = `${(element[Skeys[0]] as any).return?.key}`;
    const getNumber = (state: string): number => {
      return parseInt(state.charAt(state.length - 1));
    };
    expect(getNumber(firstPaginationKey) + 1).toEqual(getNumber(SeccondPaginationKey));
  });

  it('Should Disable Previous on Render', () => {
    const { rerender } = render(<DefaultTable />);
    let element = screen.getByTestId('pagination_true') ? screen.getByTestId('pagination_true') : ({} as any);
    const next: HTMLElement = screen.getByTestId('pagination-prev');
    const keys: string[] = [...Object.keys(element)];
    const firstPaginationKey: string = `${(element[keys[0] as string] as any).return.key}`;
    fireEvent.click(next);
    rerender(<DefaultTable />);
    element = screen.getByTestId('pagination_true');
    const Skeys = Object.keys(element);
    const SeccondPaginationKey: string = element[Skeys[0] as string].return.key;
    const getNumber = (state: string): number => {
      return parseInt(state.charAt(state.length - 1));
    };
    expect(getNumber(firstPaginationKey)).toEqual(getNumber(SeccondPaginationKey));
  });

  it('Should Naviagte to new page on Tag Click', () => {
    const { rerender } = render(<DefaultTable />);
    let element = screen.getByTestId('pagination_true') as any;
    const findTag: HTMLElement | undefined = screen.getAllByTestId('pagination_false')[0];
    const currentPagination: string = element[Object.keys(element)[0] as string].return.key;
    fireEvent.click(findTag as Element);
    rerender(<DefaultTable />);
    element = screen.getByTestId('pagination_true');
    const newPagination: string = element[Object.keys(element)[0] as string].return.key;
...
describe('Table - NoDataCustomText', () => {
  // Arguments
  const header = DefaultTable?.args?.header;
  const text = DefaultTable?.args?.customNoDataText ? DefaultTable?.args?.customNoDataText : '';
