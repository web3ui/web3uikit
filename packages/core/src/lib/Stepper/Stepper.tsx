...
const Stepper: React.FC<StepperProps> = ({
    ...
    <DivStyled id={`step-${activeStep}`}>
      <div id='stepper-title' data-testid='test-stepper_title'>
        {activeStep <= stepData.length ? (
          <Typography children={stepData[Number(activeStep - 1)]?.title || ''} variant={'h2'} />
        ) : (
          completeTitle
        )}
      </div>
      <div onClick={handleContentClick} data-testid='test-stepper_content' id='stepper-content'>
        {activeStep <= stepData.length ? (
          <Typography children={stepData[Number(activeStep - 1)]?.content} />
        ) : (
          completeMessage
        )}
      </div>
    ...
    <OrderedListStyled data-testid='test-stepper_numbers'>
      {stepData.map((step, index) => (
        <Fragment key={`step_${index}`}>
          ...
          >
            {activeStep <= Number(index + 1) ? Number(index + 1) : <Icon svg='check' fill={color.green} />}
          </ListItemStyled>
  ...
  return (
    <SectionStyled data-testid='test-stepper' {...props}>
      <HeaderStyled headerWidth={headerWidth} style={{ alignSelf: 'center' }}>
      ...
      <FooterStyled>
        {hasNavButtons && (
          <NavStyled data-testid='test-stepper_nav'>
            <Button disabled={activeStep === 0} onClick={prevStep} text='Prev' theme='secondary' type='button' />
            <Button
              disabled={activeStep === 0}
              onClick={nextStep}
              text={activeStep === stepData.length + 1 ? 'Done!' : 'Next'}
              theme='primary'
              type='button'
            />
          </NavStyled>
        ...
        {helperContent && <DivStyledHelper data-testid='test-stepper_helper'>{helperContent}</DivStyledHelper>}
      </FooterStyled>
    </SectionStyled>
  );
};
