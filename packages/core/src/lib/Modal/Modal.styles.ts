...
type TStyleProps = Pick<ModalProps, 'canOverflow' | 'fixedMode' | 'hasCancel' | 'isCentered' | 'isVisible' | 'width'>;
...
const HeaderStyled = styled.header<{
  ...
  border-bottom: ${(p) => (p.headerHasBottomBorder ? `1px solid ${color.paleBlue2}` : undefined)};
