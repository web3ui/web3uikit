  ...
  return (
    ...
    >
      <HeaderStyled data-testid={'header-test-id'}>
        {isSelected && (
          <AbsoluteIconStyled position='topL'>
            <Icon data-testid={'check-test-id'} fill={color.green} size={24} svg={iconTypes.checkmark} />
          </AbsoluteIconStyled>
        )}
      ...
      {(title || description) && (
        <FooterStyled>
          {title && (
            <Typography variant='subtitle2' data-testid={'title-test-id'} color={color.blue}>
              {title}
            </Typography>
          )}
          {description && (
            <Typography variant='caption14' data-testid={'desc-test-id'} color={color.blue}>
              {description}
            </Typography>
          )}
        </FooterStyled>
